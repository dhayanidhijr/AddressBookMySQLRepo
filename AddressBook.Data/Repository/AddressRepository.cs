...
namespace AddressBookDataLib.Repository
{
    public class AddressRepository : IAddressRepository
    {
        ...
        {
            return addressBook.AddressList.Include((item) => item.Contact).Where((item) => item.Active == 1);
        }

        public IEnumerable<Address> ReadByContactId(int contactId)
        {
            return addressBook.AddressList
                .Include((item) => item.Contact).Where((item) => (item.Contact.Id == contactId && item.Active == 1));
        }
